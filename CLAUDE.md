...
SANJI (working name; product name 千字) is a B2B SaaS product for private tutoring schools (juku) in Japan. Full context, data model, and requirements are in **`docs/spec-v2.4.md`** at the repository root. That file is long (~21 sections) — read the relevant sections for whatever issue you're working on, and re-read `§0` (How to read this document) if you're unsure which sections are binding.

## Source of truth and authority order

1. **`docs/spec-v2.4.md` is the single source of truth for product and engineering requirements.** It supersedes all prior spec versions.
