...
**Supersedes:** Product spec R1, R2; Engineering spec 1.0, 2.0, 2.1, 2.2, 2.3, 2.4; §7A design document; v2.4 and v2.5 amendment sets. **This is the single source of truth.**
...
### Changes from 2.4
- **§6.7** New provenance discipline for reproducible, human-pruned derived data assets
- **§7A.1** Five-state hypothesis model, remaining per `(student, hypothesis)` rather than per character
- **§7A.8** Confirmation thresholds now produce recommendations; only explicit instructor approval changes state
- **§10.6 / §11.10 / §11.10.1** Pending-recommendation handling, human resolution, and superseding
- **§11.4.1** Entry-path parity and evidence-quality source factors
- **§11.11–§11.14** Guardian channel scoped to v2.6; image submission dropped; voice remains client-side contextual annotation only
- **§12.1** Five-state train-car visualization bound strictly to persisted state
- **§15.8** Recommendation Integrity Gate; total build gates increased to eight
- **§16.2 / §16.3** Guardian, free-text, transcript, and audio boundaries made explicit
- **§18** H8 resequenced ahead of H4; guardian channel split into v2.6a and v2.6b
...
### 0.2 Resolved v2.5 design conflicts

The following conflicts were surfaced and resolved through founder/architect review before the v2.5 language was finalised.

#### Train-car / hypothesis-state granularity — RESOLVED: per (student, hypothesis)

The five-state visualization and the teacher-confirmation mechanism are coupled: the granularity of the state model determines whether the confirmation mechanism is operable at all.
...
#### Guardian/student self-upload and PII scope — RESOLVED across several rounds
...
#### Teacher confirmation of state transitions — RESOLVED: recommend-only AI authority
...
#### Home-practice grading — RESOLVED: selected-response, not auto-grading
...
student_hypothesis_state               -- ★ per-student diagnosis
  ...
  status enum(undetected, active, remediating, resolved_provisional, resolved_stable),
  primary key (student_id, hypothesis_id)
state_recommendation                   -- ★ recommendation; never an automatic transition
...
candidate_hypothesis                   -- ★ open path (§7A.5)
  ...
  status enum(proposed, admitted, active, recommendable, promoted, rejected),
  created_at timestamptz
diagnostics
...
| Class | Threshold to recommend `active` |
|---|---|
...
When weighted evidence on held-out probes falls below threshold, the system emits a `resolved_provisional` recommendation with a legible basis. `student_hypothesis_state.status` changes only after explicit instructor approval; recurrence likewise produces a recommendation rather than an automatic regression.
...
### 10.6 Pending recommendations at report generation (MUST)
...
- Seeds `error_profile` and hypothesis evidence; crossing a threshold emits a `state_recommendation` and does not write `student_hypothesis_state.status`
...
### 11.10 State-change recommendations (MUST)
...
`state_recommendation.basis` is populated deterministically from §7A.3 accumulation and must be legible to the instructor — evidence kanji, occurrence count, held-out probe outcome, not a bare confidence number.
...
### 11.11 Guardian accounts (v2.6, reverses §20's exclusion by named decision)
...
### 11.11.1 Guardian-facing branding (v2.6, MUST)
...
### 11.12 Home practice (v2.6)

Format is selected-response. Items render their distractors as tappable options; grading is deterministic, requiring no human and no image.
...
### 11.13 Guardian evidence submission (v2.6 — text only; image path dropped)

Guardians submit evidence from non-SENJI material as **typed text only**. Image upload was considered and dropped entirely (§0.2) — not deferred to a later phase.

### 11.14 Free-text and voice feedback (v2.6)
...
### 12.1 Student progress visualization
...
- **Resolution rate** — proportion of `remediating` states receiving and obtaining approval for `resolved_provisional` recommendations on held-out re-probe. **This is the direct measure of whether remediation works**
- Candidate hypothesis proposal, admission, and promotion counts
- Pending recommendation count and age distribution, per branch and instructor
- Approval / rejection / pending ratio per hypothesis — a hypothesis rejected at a consistently high rate indicates threshold miscalibration and is reviewed by a human, never auto-adjusted
...
Eight build-breaking CI gates. Each exists because the corresponding failure is silent and reaches a customer before it reaches the team.
...
- **B:** every report either carries ≥1 confirmed diagnosis **or** sets `insufficient_evidence: true`. A pending recommendation or below-threshold finding rendered as confident fails the build
...
**Extended to §11.4.1:** assert the extraction adapter's outbound payload contains no hypothesis state, no prior entries, and no student or guardian identifier.

**Extended to §11.14.1:** assert no audio blob is written to storage or transmitted to any endpoint; no guardian free-text or transcript reaches any LLM client, including from report generation, summarisation, or analytics paths; voice-attributed feedback records carry either an explicitly selected `hypothesis_id` or null, never kanji-derived attribution.
...
- **State-write isolation** — no code path writes `student_hypothesis_state.status` except the recommendation-approval handler. Fixtures attempt state writes from the evidence accumulator, the report generator, and the diagnostic scorer; all must fail
...
### 15.8 Recommendation Integrity Gate
...
§15.9 (a submission pre-check gate considered during the image-upload discussion) is explicitly NOT written — image upload was dropped from scope, not merely deferred.

Total gates: eight (v2.4's seven, plus §15.8).
...
No guardian `phone_e164` is transmitted under any circumstance. No audio is transmitted to any external provider.
...
| CI | GitHub Actions — eight build gates plus the §6.5 hash workflow |
...
#### v2.6 — guardian channel, two phases
...
- **Five-state display labels** — replace the §7A.1 Japanese placeholders before they appear in any user-facing surface
...
### 19.14 Guardian portal domain strategy — v2.6a decision

Per-organisation subdomain vs. neutral apex domain remains open. A SENJI-branded URL defeats §11.11.1 white-labelling regardless of on-page theming.

---

## 20. Explicitly out of scope for V1

Stroke-order recognition or scoring · handwriting input · stroke-form error detection · student login · guardian channel (v2.6) · image submission (dropped entirely, not deferred) · email delivery to parents · gamification · subjects other than kanji · native mobile app · offline mode · franchise 本部 hierarchy · vertical writing templates (V1.1) · **support chatbot** · **AI intent classification or confidence-threshold support routing** · **automated support escalation** · **LINE公式アカウント** · confidence intervals and adaptive probe sequencing (V1.1) · per-hypothesis mastery curves (V1.1) · analytics beyond §13 · usage-based customer billing · consumer-facing product
...
3. **All eight build gates green** (§15): Specificity, PII Privacy, Font Integrity, Tenant Isolation, Classifier Determinism, Targeting Fidelity, Clinical Language, Recommendation Integrity
...
8. **Held-out re-probe demonstrated end to end** — the system emits a correct `resolved_provisional` recommendation, with legible basis, when held-out re-probe evidence clears threshold, and no state change occurs absent approval (§7A.9)
