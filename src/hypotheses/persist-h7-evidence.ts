...
interface H7ResultRow extends Record<string, unknown> {
  id: string;
  org_id: string;
  branch_id: string;
  student_id: string;
...
export interface H7EvidenceInput {
  resultId: string;
  itemType: string;
...
export interface PersistH7EvidenceInput {
  orgId: string;
  branchId: string;
  studentId: string;
  resultId: string;
...
export interface PersistedH7Evidence {
  id: string;
  inserted: boolean;
}
...
export function computeH7Evidence(input: H7EvidenceInput): WeightedEvidenceRow | null {
  ...
  if (classifyH7(input.lexicalSurface, input.responseText).classification !== "H7") return null;
  return {
    kanji: input.targetKanji,
    weight: SOURCE_FACTORS[input.sourceKind as SourceKind],
    evidenceId: `${input.resultId}:H7`,
  };
}

function validateRequest(input: PersistH7EvidenceInput): void {
  for (const [name, value] of Object.entries(input)) {
    if (typeof value !== "string" || !UUID.test(value)) throw new Error(`${name} must be a UUID`);
  }
}

/** Read one existing result under RLS and persist at most one H7 evidence row. */
export async function persistH7Evidence(
  database: ResultDatabase,
  input: PersistH7EvidenceInput,
): Promise<PersistedH7Evidence | null> {
  ...
  try {
    ...
    const result = await client.query<H7ResultRow>(
      `select r.id, s.org_id, s.branch_id, r.student_id,
              r.is_correct, r.wrong_answer_text as response_text,
              r.graded_at as observed_at, r.source_kind,
              i.item_type, i.target_kanji, i.lexical_surface
       from results r
    ...
    if (!existing) throw new Error("H7 evidence insert did not produce a durable row");
    await client.query("commit");
