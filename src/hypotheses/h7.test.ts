...
for (const [surface, correct, characterByCharacter, sourcePage] of pilots) {
  test(`${surface}/${correct} resolves before character-level fallback`, () => {
    ...
    assert.deepEqual(classifyH7(surface, characterByCharacter), {
      classification: "H7",
      expected_readings: [correct],
      source_page: sourcePage,
    });
  });
}

test("明日/あした exposes a source-coverage gap instead of silently passing", () => {
  assert.deepEqual(classifyH7("明日", "あした"), {
    classification: "H7",
    expected_readings: ["あす"],
    source_page: 50,
  });
});
